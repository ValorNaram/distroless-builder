...
* *Possible Values:*
** `yes`: Execute the pipeline inside a container. Recommended for most scenarios e.g. build or development machines.
** `no`: Execute the pipeline on the host system. Only recommended when the `distroless-builder.sh` script is being executed inside a container.
* *Default Value:* `yes`
...
* *Possible Values:*
** `build`: Builds the image from the source located at `COLLECTOR_RESOURCE_PATH`.
...
* *Possible Values:* Any valid container image reference (e.g., `docker.io/library/debian:12`, `docker.io/library/ubuntu:latest`).
...
[source,bash]
----
WORKER_CONTAINER_IMAGE_BUILD_USE_BASE_IMAGE="docker.io/library/ubuntu:22.04"
WORKER_CONTAINER_IMAGE_BUILD_USE_BASE_IMAGE="docker.io/library/alpine:latest"
----

== `WORKER_CONTAINER_IMAGE_NAME` / `--worker-container-image-name`

Defines the image repository name to be used or created for the pipeline. The image must contain all necessary dependencies (e.g., python3) to execute the scripts. This is only relevant if `USE_CONTAINERS` is set to `yes`.
...
Defines the tag for the image specified in `WORKER_CONTAINER_IMAGE_NAME`. This is only relevant if `USE_CONTAINERS` is set to `yes`.
...
Sets the runtime name of the worker container instance. This is the name used to start, stop, and execute commands within the container. This is only relevant if `USE_CONTAINERS` is set to `yes`.
...
== `COLLECTOR_RESOURCE_PATH` / `--collector-resource-path`

Specifies the file system path to the directory containing the resources required to build the worker container (specifically the `Containerfile`) and the resources needed by the collector. This directory serves as the build context for the container when `USE_CONTAINERS=yes`. It is also being used when `USE_CONTAINERS=no` as it is the location of the collector.

* *Possible Values:* A relative or absolute path to a directory.
* *Default Value:* `collector_resources`

.CLI Argument Examples
[source,bash]
----
./distroless-builder.sh --collector-resource-path=./docker-context
./distroless-builder.sh --collector-resource-path=/opt/dependency-collector/resources
----

.Configuration File Examples
[source,bash]
----
COLLECTOR_RESOURCE_PATH="docker"
COLLECTOR_RESOURCE_PATH="../container-files"
----

== `REMOVE_WORKER_CONTAINER_AT_END` / `--remove-worker-container-at-end`

Determines if the worker container should be removed after the script finishes execution. If set to `no`, the container is stopped but persists. This is only relevant if `USE_CONTAINERS` is set to `yes`.
...
This argument can be left empty. In this case the collector , responsible to collect the application and its dependencies which includes installing, will not utilize the package management to install packages.
...
== DISTROLESS_BUILDER_DIR / (None)

A helper variable available only within a configuration file. It contains the full path to the directory of the `distroless-builder.sh` script.
...
== `SOURCE_FOLDER_ON_SCRIPT_HOST` / `--source-folder-on-script-host

Source folder on script host containing directories and files to copy inside the container. The operation will be performed directly after starting the container. Applies only if `USE_CONTAINERS=yes`

* *Possible Values:* absolute path to the folder to copy its contents inside the container. Relative paths are relative to the current working dir
* *Default Value:* `""` (Empty string)

.CLI Argument Examples
[source,bash]
----
./distroless-builder.sh --source-folder-on-script-host="path/to/source"
----

.Configuration File Examples
[source,bash]
----
# specify source folder to copy its content to container e.g. files needed for compilation inside container.
SOURCE_FOLDER_ON_SCRIPT_HOST="${DIRECTORY_OF_CONFIG_FILE}/source"
----